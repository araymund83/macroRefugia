require(pacman)
pacman::p_load(ggplot2, glue, googledrive, qs, RColorBrewer,terra,
               sf, tidyterra, tidyverse)

g <- gc(reset = TRUE)
rm(list = ls())

# Load data ----------------------------
path <- './outputs/velocity/treeVelRas'
...
makeVelMap <- function(sp, type, baseline ){
  #sp <- species[8]
  ...
  spMap <- map(.x = 1:length(ssp), function(k){
    ...
    spMap<- ggplot() +
      ...
      scale_fill_gradientn(colours = brewer.pal(n = 5, name = 'Accent'), 
                           na.value = NA) +
      theme_bw() + 
      ...
      labs(x = 'Longitude', y = 'Latitude', fill = 'Velocity',
           title = glue('{str_to_title(type)} velocity for: {sp}'),
           subtitle = labs[k]) +
      coord_sf(xlim = c(-4500000, 3500000), 
               ylim = c(-1500000, 5500000),
               expand = FALSE)  
    out <- glue('./maps/trees2/velocity')
    ifelse(!file.exists(out), dir_create(out), print('Already exists'))
    
    ggsave(plot = spMap, filename = glue('{out}/{type}_velocity_{sp}_{ssp[k]}_{baseline}.png'), 
...
# Apply the function ------------------------------------------------------
dfrm <- map(.x = species, type = type, baseline = 'p1991', .f = makeVelMap)


# Upload to Drive----------------------------------------------------------
outputFolder <- './maps/trees2/Velocity'
googleFolderID <- 'https://drive.google.com/drive/folders/1rxCtridri8T1yA03gZdvdQgR_wUcH5t0' #Trees

fl <- list.files(outputFolder, pattern = 'forwardVel', full.names = TRUE)
